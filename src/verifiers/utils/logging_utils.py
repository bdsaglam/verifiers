...
def is_field_eligible(k, v):
    return not isinstance(v, dict) and k not in ["prompt", "docs"]
...
def print_prompt_completions_sample(
    ...
    # Add columns
    table.add_column("Input", style="bright_blue")
    table.add_column("Prompt", style="bright_yellow")
    table.add_column("Completion", style="bright_green")
    table.add_column("Reward", style="bold cyan", justify="right")

    for input, prompt, completion, reward in zip(inputs, prompts, completions, rewards, strict=True):
        ...
        # Concat all string fields in input
        input_str = "\n\n".join([f"{k}: {str(v)[:1000]}" for k, v in input.items() if is_field_eligible(k, v)])

        table.add_row(Text(input_str), Text(prompt), formatted_completion, Text(f"{reward:.2f}"))
