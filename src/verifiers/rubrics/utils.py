...
def get_assistant_messages(trajectory: List[Dict[str, str]]) -> List[Dict[str, str]]:
    """Helper function to extract assistant messages from a trajectory."""
    return [msg for msg in trajectory if msg["role"] == "assistant"]


def get_last_assistant_message(trajectory: List[Dict[str, str]]) -> str | None:
...
def get_last_answer(trajectory: List[Dict[str, str]], tag: str = "answer") -> str | None:
